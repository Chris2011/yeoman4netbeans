...
function runGenerator() {
    debug('invoke run');
    env.run(args, optionsForRun).then(callbackDone, err => {
        console.log("Error in runGenerator: ", err)
    });
}
...
function Adapter() {
    ...
    if (options.newYoVersion) {
        this.prompt = function (questions, answers, cb) {
            ...
            if (typeof cb === 'function') {
                debug("Run original callback for " + (arguments.length > 0 ? JSON.stringify(arguments[0]) : ""));
...
//                    }
//                } catch (err) {
//                    console.log("Error in Adapter: ", err)
//                }
//            });
//
//            return promise;
